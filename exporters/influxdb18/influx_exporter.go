...
func (t *Exporter) Write(_ context.Context, spans []*klogga.Span) error {
	points := make([]influxClient.Point, 0, len(spans))
	...
	for _, span := range spans {
		...
		fields[constants.SpanID] = span.ID().String()
		fields[constants.TraceID] = span.TraceID().String()
...
func (t *Exporter) measurement(span *klogga.Span) string {
